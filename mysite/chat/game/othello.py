...
class OthelloEnv(gym.Env):
    global BLACK_DISK, NO_DISK, WHITE_DISK, IMAGE_SIZE, MAX_INT, WINDOW_H, WINDOW_W, BOARDFIELD
    ...
    def __init__(self,
        ...
        self.rand_state = self.env.rand_state
        self.socket = socket
    ...
    def render(self, mode='np_array', close=False,socket=None):
        self.env.render(mode=mode, close=close, socket=self.socket)
...
class OthelloBaseEnv(gym.Env):
    ...
    def _reset_board(self):
        ...
        if(self.rand_board_count > 0):
            copy_board_state = np.reshape(board_state, [1,self.board_size*self.board_size])
            random_board =list(range(0,self.board_size*self.board_size))
            random_board.remove((center_row_ix) * self.board_size +center_col_ix)
            random_board.remove((center_row_ix) * self.board_size + center_col_ix - 1)
            random_board.remove((center_row_ix - 1) * self.board_size + center_col_ix)
            random_board.remove((center_row_ix - 1) * self.board_size + center_col_ix - 1)
            ix = sample(range(0,self.board_size*self.board_size-5),self.rand_board_count)
            for i in ix:
                copy_board_state[0][random_board[i]] = -2
                (self.rand_state).append(random_board[i])
            board_state = np.reshape(copy_board_state, [self.board_size,self.board_size])
        return board_state
    ...
    def print_board(self, print_valid_moves=True,socket=None):
        ...
        temp.append('Turn: {}'.format(
            'WHITE' if self.player_turn == WHITE_DISK else 'BLACK'))
        #print('Valid actions: {}'.format(valid_actions))
        temp.append('Valid actions: {}'.format(valid_actions))
        for row in board:
            #print(' '.join(map(lambda x: ['B', 'O', 'W', 'V'][x + 1], row)))
            temp.append(' '.join(map(lambda x: ['B', 'O', 'W', 'V'][x + 1], row)))
        #print('-' * 10)
        temp.append('-' * 10)
        temp = '\n'.join(temp)
        #print(temp)
        asyncio.create_task(consumer.ChatConsumer.send_gameboard(socket,board = temp))
        #consumer.ChatConsumer.send_gameboard(socket,board = temp)
            ...
            if len(self.possible_moves) == 0:
                ...
                if len(self.possible_moves) == 0:
                    if not self.mute:
                        ...
                        asyncio.create_task(consumer.ChatConsumer.send_gameboard(self.socket,'No possible moves for either party.'))
    ...
    def determine_winner(self, sudden_death=False):
        ...
        if sudden_death:
            if not self.mute:
                ...
                asyncio.create_task(consumer.ChatConsumer.send_gameboard(self.socket,'sudden death due to rule violation'))
            if self.player_turn == WHITE_DISK:
                if not self.mute:
                    print('BLACK wins')
                    asyncio.create_task(consumer.ChatConsumer.send_gameboard(self.socket,'BLACK wins'))
                return BLACK_DISK
            else:
                if not self.mute:
                    print('WHITE wins')
                    asyncio.create_task(consumer.ChatConsumer.send_gameboard(self.socket,'WHITE wins'))
                return WHITE_DISK
        else:
            white_cnt, black_cnt = self.count_disks()
            if not self.mute:
                print('white: {}, black: {}'.format(white_cnt, black_cnt))
                asyncio.create_task(consumer.ChatConsumer.send_gameboard(self.socket,'white: {}, black: {}'.format(white_cnt, black_cnt)))
            if white_cnt > black_cnt:
                ...
                if not self.mute:
                    print('DRAW')
                    asyncio.create_task(consumer.ChatConsumer.send_gameboard(self.socket,'DRAW'))
                return NO_DISK
    ...
    def render(self, mode='np_array', close=False, socket=None):
        if close:
            return
        if mode == 'np_array':
            self.print_board(socket=socket)
        '''else:
            self.show_gui_board()'''
    ...
    def draw_board(self):
        ...
        for i in range(self.board_size):
            for j in range(self.board_size):
                ...
                elif self.board_state[i][j] == WHITE_DISK:
